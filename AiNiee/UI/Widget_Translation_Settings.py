...
from .Widget_Translation_Settings_A import Widget_Translation_Settings_A
from .Widget_Translation_Settings_B import Widget_Translation_Settings_B
...
class Widget_Translation_Settings(QFrame):  # 翻译设置主界面
    def __init__(self, text: str, parent=None):  # 构造函数，初始化实例时会自动调用
        ...
        self.A_settings = Widget_Translation_Settings_A(
            "A_settings", self
        )  # 创建实例，指向界面
        self.B_settings = Widget_Translation_Settings_B(
            "B_settings", self
        )  # 创建实例，指向界面
