...
class Widget_Openai(QFrame):  #  Openai账号界面
    def __init__(self, text: str, parent=None):  # 解释器会自动调用这个函数
        ...
        box_account_type.setStyleSheet(
        ...
        self.comboBox_account_type.addItems(
            [
                "免费账号",
                "付费账号(等级1)",
                "付费账号(等级2)",
        ...
        self.comboBox_account_type.setCurrentIndex(
            0
    ...
    def test_request(self):
        if Global.Running_status == 0:
            # 创建子线程
            thread = Global.background_executor("openai官方接口测试")
            thread.start()
