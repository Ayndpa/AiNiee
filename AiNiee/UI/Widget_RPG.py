...
from .Widget_Export_Source_Text import Widget_Export_Source_Text
from .Widget_Import_Translated_Text import Widget_Import_Translated_Text
...
class Widget_RPG(QFrame):  # RPG提取导入主界面
    def __init__(self, text: str, parent=None):  # 构造函数，初始化实例时会自动调用
        ...
        self.A_settings = Widget_Export_Source_Text(
            "A_settings", self
        )  # 创建实例，指向界面
        self.B_settings = Widget_Import_Translated_Text(
            "B_settings", self
        )  # 创建实例，指向界面
