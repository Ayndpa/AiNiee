from PyQt5.QtWidgets import QFrame, QLabel, QHBoxLayout, QVBoxLayout, QGroupBox
...
class Widget_Export_Source_Text(QFrame):  #  导出子界面
    def __init__(self, text: str, parent=None):  # 解释器会自动调用这个函数
        ...
        box_start_export.setStyleSheet(
            """ QGroupBox {border: 0px solid lightgray; border-radius: 8px;}"""
        )  # 分别设置了边框大小，边框颜色，边框圆角
        layout_start_export = QHBoxLayout()

        # 设置“开始翻译”的按钮
        self.primaryButton_start_export = PrimaryPushButton(
            "开始导出", self, FIF.UPDATE
        )
