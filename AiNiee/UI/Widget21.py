...
class Widget21(QFrame):  # 原文替换字典界面
    def __init__(self, text: str, parent=None):  # 解释器会自动调用这个函数
        ...
        self.label2.setText(
            "(在翻译开始前，根据表格中内容，将游戏文本中出现的原文全部替换成为译文，再发送过去翻译)"
        )
        ...
        self.label4.setText(
            "(在翻译进行时，如果该次游戏文本出现了部分字典原文，则会将这部分字典内容作为AI的翻译示例，一并发过去翻译)"
        )
    ...
    def Exporting_dictionaries(self):
        ...
        # 将字典保存到文件中
        with open(
            os.path.join(Output_Folder, "用户替换字典.json"), "w", encoding="utf-8"
        ) as f:
